...
struct Thread {
    ...
    static std::expected<Thread*, rlib::Error> make(rlib::Allocator& allocator, AddressSpace addressSpace, std::uint64_t entryPoint, std::uintptr_t stackTop);
...
public:
    ...
    std::expected<Thread*, rlib::Error> createThread(AddressSpace addressSpace, std::uint64_t entryPoint, std::uintptr_t stackTop); 
