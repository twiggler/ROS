...
namespace rlib {
    ...
    public:
        ...
        InputStream slice(std::size_t start, std::size_t size) const
        requires IsSlicable<Source>;
    ...
    public:
        explicit StreamRange(InputStream<Source>& stream) : stream(&stream) {}

        auto begin() const { return InputStreamIterator<T, Source>(*stream); }
    ...
    inline MemorySource::MemorySource(std::byte* start, std::size_t size) : data(start), size(size), pos(0) {}
    ...
    template<class Source>
    InputStream<Source>::InputStream(Source buffer) : source(std::move(buffer))
    {}

    template<class Source>
    InputStream<Source>& InputStream<Source>::seek(std::size_t pos)
    {
        if (!ok()) {
            return *this;
        }
    ...
    template<class Source>
    std::size_t InputStream<Source>::position() const
    {
        return source.position();
    }

    template<class Source>
    std::optional<Error> InputStream<Source>::error() const
    {
        return lastError;
    }

    template<class Source>
    bool InputStream<Source>::ok() const
    {
        return !lastError;
    }

    template<class Source>
    bool InputStream<Source>::eof() const
    {
        return lastError == EndOfStream;
    }

    template<class Source>
    InputStream<Source> InputStream<Source>::slice(std::size_t start, std::size_t size) const
    requires IsSlicable<Source>
    {
    ...
    template<IsStreamReadable T, class Source>
    InputStreamIterator<T, Source>::InputStreamIterator(InputStream<Source>& stream) : stream(&stream)
    {
