...
namespace rlib::intrusive {
    ...
    public:
        ...
        void insertAfter(ListNode& after)
        {
            remove();
            _next        = after._next;
            _prev        = &after;
        ...
        void setOwner(T* element) { _owner = element; }

        ListNode* next() const { return this == _next ? nullptr : _next; }

        ListNode* prev() const { return this == _prev ? nullptr : _prev; }
    ...
    public:
        ...
        List(List&& other) : head(std::move(other.head)) { other.head = nullptr; }

        void push_front(T& element)
        {
            auto& node = NodeGetter::get(&element);
            node.insertAfter(*head);
            node.setOwner(&element);
        }
